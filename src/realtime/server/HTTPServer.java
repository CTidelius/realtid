...
import java.io.IOException;                   // Provides InputStream, OutputStream
...
public class HTTPServer extends Thread {
     ...
     */
    public HTTPServer(int port, CameraServer server) {
        myPort   = port;
        myCamera = server;
    }
        ...
        while (true) {
            try {
                ...
                if (request.substring(0,4).equals("GET ")) {
                    ...
                    byte[] img = myCamera.getLastImage();
                    int length = ByteBuffer.wrap(img, 0, 4).getInt();
                    os.write(img, 4, length);
                }
                else {
    ...
    private CameraServer myCamera;                      // Makes up the JPEG images
